    ...
    if (inputFile.open(QIODevice::ReadOnly)) {
        ...
        while (!inputFile.atEnd()) {
            ...
            if (bookInfo.size() == 2 && bookInfo[1].trimmed() == title) {
                ...
                QDateTime dueDate = currentDate.addSecs(60); // Добавление 1 минуту (60 сек)
...
int main(int argc, char* argv[]) {
    ...
    app.setWindowIcon(QIcon(":/new/prefix1/icon.ico"));
    QWidget mainWidget;
    ...
    pal.setBrush(QPalette::Background, QBrush(QPixmap("D:/multimedia/Desktop/APM/lib.jpg")));
